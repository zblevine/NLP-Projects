...
public class better_tag {
	...
	public static HashMap<List<String>, Double> getTau(List<String[]> input) {
		HashMap<String, Integer> singleCounts = new HashMap<String, Integer>();
		HashMap<List<String>, Integer> doubleCounts = new HashMap<List<String>, Integer>();
		HashMap<String, Integer> tauCounts = new HashMap<String, Integer>();
		HashMap<String, String> tauMap = new HashMap<String, String>();
		...
		for(int i = 0; i < input.size(); i++) {
			...
			for(int j = 1; j < sentence.length; j += 2) {
				...
				if(tauCounts.containsKey(sentence[j-1])) {
					tauCounts.put(sentence[j-1], tauCounts.get(sentence[j-1]) + 1);
				}
				else {
					tauCounts.put(sentence[j-1], 1);
					tauMap.put(sentence[j-1], sentence[j]);
				}
			}
		}
		
		Iterator<String> tauIter = tauCounts.keySet().iterator();
		while(tauIter.hasNext()) {
			String cw = tauIter.next();
			if(tauCounts.get(cw) == 1) {
				List<String> currPair = new ArrayList<String>(2);
				currPair.add(tauMap.get(cw));
				currPair.add("*UNK*");
				if(doubleCounts.containsKey(currPair)) {
					doubleCounts.put(currPair, doubleCounts.get(currPair) + 1);
				}
